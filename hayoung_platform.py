...
def get_conn():
    ...
    conn.execute("PRAGMA journal_mode=WAL")   # WAL 모드: 동시 읽기/쓰기 충돌 방지
    return conn
...
def send_kakao_alimtalk(phone: str, school: str, food_kg: float, total_price: int) -> bool:
    ...
    try:
        # 솔라피(Solapi) 카카오 알림톡 공식 API (https://developers.solapi.com)
        # .env에 KAKAO_API_KEY=솔라피_API_KEY, KAKAO_SENDER_KEY=솔라피_API_SECRET 입력
        ...
        res = requests.post(
            ...
            json={
                "message": {
                    "to":   phone.replace("-", ""),
                    "from": os.getenv("SENDER_PHONE", ""),  # .env에 SENDER_PHONE=발신번호
                    "kakaoOptions": {
                        "pfId":         os.getenv("KAKAO_PF_ID", ""),  # 카카오 채널 ID
                        "templateId":   os.getenv("KAKAO_TEMPLATE_ID", "HAYOUNG_COLLECT_01"),
                        "variables": {
                            "#{학교명}":   school,
                            "#{수거량}":   f"{food_kg:,.0f}",
                            "#{청구금액}": f"{total_price:,}"
            ...
            headers={
                "Authorization": f"HMAC-SHA256 apiKey={KAKAO_API_KEY}, date={date_str}, salt={salt}, signature={signature}",
...
def create_secure_excel(df, title):
    if not EXCEL_PASSWORD:
        ...
        _st.warning("⚠️ .env 파일에 EXCEL_PASSWORD가 설정되지 않아 시트 보호가 적용되지 않습니다.")
    output = io.BytesIO()
...
KAKAO_SENDER_KEY=솔라피_API_SECRET
KAKAO_PF_ID=카카오채널_플러스친구ID
KAKAO_TEMPLATE_ID=등록한_템플릿ID
SENDER_PHONE=발신번호(010-XXXX-XXXX)
        """, language="bash")
