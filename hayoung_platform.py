...
# ── [신규 3단계] 전자 계약함 화면 ───────────────────────────────
        with school_tab5:
            ...
            if not pending_contracts:
                st.write("현재 검토 대기 중인 계약서 초안이 없습니다.")
            else:
                for row in pending_contracts:
                    c_id, c_start, c_end, c_waste, c_unit, c_status, c_memo = row
                    
                    # st.container: 화면에 테두리 박스를 예쁘게 그려주는 기능
                    with st.container(border=True):
                        st.subheader(f"📄 {c_waste} 수집·운반 위수탁계약 (초안)")
                        st.write(f"- **계약 기간**: {c_start} ~ {c_end}")
                        st.write(f"- **단가**: {c_unit}원/L")
                        st.write(f"- **현재 상태**: `{c_status}`")
                        
                        if c_memo:
                            st.caption(f"💡 본사 메모: {c_memo}")
                        
                        col1, col2 = st.columns(2)
                        with col1:
                            # 행정실에서 '승인' 버튼을 눌렀을 때 작동하는 부분
                            if st.button("✅ 서명완료 (초안 승인)", key=f"app_{c_id}", type="primary", use_container_width=True):
                                school_reply_contract(c_id, "서명완료")
                                st.success("계약이 성공적으로 승인되었습니다! 본사로 결과가 전송됩니다.")
                                time.sleep(1) # 1초 대기
                                st.rerun() # st.rerun: 화면 새로고침
                        
                        with col2:
                            # 행정실에서 '반려' 버튼을 눌렀을 때 작동하는 부분
                            reject_reason = st.text_input("수정 요청 사유 (선택사항)", key=f"rej_txt_{c_id}", placeholder="예: 단가 조정 요청")
                            if st.button("✍️ 수정 요청 (반려)", key=f"rej_{c_id}", use_container_width=True):
                                school_reply_contract(c_id, "반려", reject_reason)
                                st.warning("수정 요청이 본사로 전달되었습니다.")
                                time.sleep(1)
                                st.rerun()
        # ────────────────────────────────────────────────────────────
